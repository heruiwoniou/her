...
export default function entryFactory(vueSetting, baseOption) {
  let createApp = async function(option) {
    global.__router__ = option.router;
    global.__store__ = option.store;

    instance = global.__vm__ = new Vue(option);
    ...
    option.router.afterEach(async function(to, from) {
      let layout = to.matched[0].components.default.options
...
async function loadAsyncComponents(to, from, next) {
  try {
    await Promise.all(
      to.matched.map(async (m, index) => {
        try {
          await Promise.all(
            Object.keys(m.components).map(async key => {
