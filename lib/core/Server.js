...
export default class Server {
  ...
  async buildWebpack() {
    ...
    this.compilers = [this.webpackConfig].map(compilersOption => webpack(compilersOption))

    await Promise.all(this.compilers.map(compiler => new Promise(async (resolve, reject) => {
      // --- Dev Build ---
      let timer
      compiler.plugin('done', async (stats) => {
        clearTimeout(timer)
        timer = setTimeout(() => process.nextTick(resolve), 5000)
      })
